...
const createCourse = catchAsync(async (req, res, next) => {
    const { courseCode, courseName, year,creditHours, class: classId, currentTeacher } = req.body;

    // Validate required fields
    if (!courseCode || !courseName || !year || !creditHours || !classId || !currentTeacher) {
...
const getCourse = async (req,res)=>{
  ...
  const updateCourse = async (req,res)=>{
    ...
    const { courseCode, courseName, year , creditHours , classes } = req.body;
...
// get class and course for teacher
