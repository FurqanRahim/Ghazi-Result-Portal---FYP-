...
exports.updateTeacher = async (req, res) => {
  ...
  try {
    ...
    // const isReferenced = await Course.exists({ "classes.teacher": id });
    // if (isReferenced) {
    //   return res.status(400).json({
    //     message:
    //       "This teacher is referenced in the Course schema. Please update or remove the reference before modifying the record.",
    //   });
    // }
    ...
    if (password) {
      ...
      teacher.password = password; // Hashing will be handled by pre-save middleware
    }
