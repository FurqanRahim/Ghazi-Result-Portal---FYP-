...
const exportEnrollmentsToExcel = catchAsync(async (req, res, next) => {
    ...
    const csvData = enrollments.map((enrollment) => {
        ...
        const courseNames = enrollment.course
            ...
            const creditHours = enrollment.course
            .map((course) => course.creditHours) // Extract course names
            .join(",");
    ...
    //     "Registration Number": enrollment.student.reg_No,
    //     "Course Code": enrollment.course.courseCode,
    //     "Course Name": enrollment.course.courseName,
    //     "Session": enrollment.course.year,
    //     "Class Name": enrollment.class.className,
    //     "Class Code": enrollment.class.classCode,
...
const getStudentCourses = async (req,res)=>{
    try {
        ...
        // console.log(courses)
        const totalCourses = courses.reduce((sum, enrollment) => sum + enrollment.course.length, 0);
...
module.exports = { enrollStudentByRegistration, getAllEnrollments,
    exportEnrollmentsToExcel, getEnrollments, updateEnrollment ,
     deleteEnrollment,getEnrollment, getStudent , getStudentCourses , getStudentCourseYears , getStdClassCourses};
